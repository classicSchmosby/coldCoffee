...
<html lang="en">
  ...
  <body>
  <?php 
  	$locationQuery = mysqli_query($conn, 
  		"SELECT * FROM location WHERE (locationId = 69) ORDER BY locationId asc");
  	$locationQueryDisp = mysqli_fetch_assoc($locationQuery);
  ?>
    <div class="container">
		<div class="row">
			<div class="col-sm-3">
				<div class="rating-block">
					...
					<h4>
						<?php echo(($locationQueryDisp["locationId"]) . ' - ' . ($locationQueryDisp["companyName"])); ?>&nbsp;
						<i id="globeIcon" class="glyphicon glyphicon-globe" title="Find on the Map" onclick="findMap()"></i>
					</h4>

					<h5>
						<?php echo($locationQueryDisp["postcode"]); ?> - 
						<?php echo($locationQueryDisp["phoneNumber"]); ?>
					</h5>
					...
					<h2 class="bold padding-bottom-7">
						<span id="mainAvgRating" name="mainAvgRating">
							<?php 
								$averageRatingSet = ($locationQueryDisp["averageRating"]);
								$reviewAvgQuery = mysqli_query($conn, 
									"SELECT AVG(ratingOverall) AS avgJoe FROM rating WHERE FK_locationId = 69");
								$row = mysqli_fetch_array($reviewAvgQuery);
								$rowDecFormatting = ($row[0]);
								echo(number_format((float)$rowDecFormatting, 2, '.', ''));
							?>
						</span>
						/<small> 5</small>
					</h2>
					<button id="setRatingOne" name="setRatingOne" type="button" class="btn btn-default btn-grey btn-sm" aria-label="Left Align">
					  <span class="glyphicon glyphicon-star2" aria-hidden="true"></span>
					</button>
					<button id="setRatingTwo" name="setRatingTwo" type="button" class="btn btn-default btn-grey btn-sm" aria-label="Left Align">
					  <span class="glyphicon glyphicon-star2" aria-hidden="true"></span>
					</button>
					<button id="setRatingThree" name="setRatingThree" type="button" class="btn btn-default btn-grey btn-sm" aria-label="Left Align">
					  <span class="glyphicon glyphicon-star2"></span>
					</button>
					<button id="setRatingFour" name="setRatingFour" type="button" class="btn btn-default btn-grey btn-sm" aria-label="Left Align">
					  <span class="glyphicon glyphicon-star2" aria-hidden="true"></span>
					</button>
					<button id="setRatingFive" name="setRatingFive" type="button" class="btn btn-default btn-grey btn-sm" aria-label="Left Align">
					  <span class="glyphicon glyphicon-star2" aria-hidden="true"></span>
					</button>
				</div> <!-- rating-block -->
			</div> <!-- .col-sm-3 -->
			...
			<div class="col-sm-7">
				...
				<div class="review-block">
					...
					<div class="input-group">
  						...
  						<input id="reviewTitle" name="reviewTitle" type="text" class="form-control" placeholder="(optional) Add a title!" aria-describedby="basic-addon1" />
					</div>
					<p></p>
					...
					<div class="input-group">
						<span class="input-group-addon" id="basic-addon1"><i class="glyphicon glyphicon-comment"></i></span>
  						...
  						<textarea id="reviewDesc" name="reviewDesc" type="text" class="form-control" placeholder="(optional) Add a comment!" aria-describedby="basic-addon1"></textarea>
					</div>
				</form>
					<!-- </FORMAREA> -->
				<hr/>
				...
				$reviewQuery = mysqli_query($conn,
					"SELECT * FROM rating WHERE (FK_locationId = 69) ORDER BY datePosted DESC LIMIT 3");
				while ($reviewQueryDisp = mysqli_fetch_assoc($reviewQuery)): ?>
					<div class="row">
						<div class="col-sm-3" style="text-align:center;">
							<img src="assets/img/user.png" style="width:60px;height:60px;" class="img-rounded">
